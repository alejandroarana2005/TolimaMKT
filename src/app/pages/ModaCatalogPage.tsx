...
const municipiosConProductos = (() => {
  const ids = [...new Set(productosModa.map((p) => p.municipio))];
  return ids
    .map((id) => ({ id, nombre: getMunicipio(id)?.nombre ?? id }))
    .sort((a, b) => a.nombre.localeCompare(b.nombre));
})();

export default function ModaCatalogPage() {
  useEffect(() => { document.title = "Moda y Accesorios — Catálogo | TolimaMKT"; }, []);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
      ...
      <FilterSection title="Categorías">
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {CATEGORIAS.map((cat) => (
            <div key={cat} style={{ display: "inline-flex", alignSelf: "flex-start" }}>
              <TagCategoria
                label={cat}
      ...
      <FilterSection title="Municipio">
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {municipiosConProductos.map((m) => (
            <div key={m.id} style={{ display: "inline-flex", alignSelf: "flex-start" }}>
      ...
      <FilterSection title="Precio (COP)">
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          <button
            ...
            style={{
              height: "34px", background: "#F4F3F0",
              border: "1px solid #E5E4E0", borderRadius: "8px",
              fontSize: "13px", fontWeight: 600, color: "#2C2C2A",
    ...
    <div style={{ minHeight: "100vh", background: "#FFFFFF", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        .mcat-topbar {
          ...
          background: #FFFFFF; border-bottom: 1px solid #F0EFE9;
        }
        .mcat-topbar-inner { max-width: 1440px; margin: 0 auto; padding: 14px 24px; }
        .mcat-layout {
        ...
        .mcat-sidebar {
          ...
          background: #FFFFFF; border: 1px solid #F0EFE9;
        ...
        .mcat-mobile-btn { display: none; }
        .mcat-sheet-backdrop {
          position: fixed; inset: 0;
          background: rgba(44,44,42,0.45); backdrop-filter: blur(4px);
          z-index: 100; animation: mcat-fadeIn 200ms ease;
        }
        .mcat-sheet {
          position: fixed; bottom: 0; left: 0; right: 0;
          background: #FFFFFF; border-radius: 20px 20px 0 0;
          padding: 24px 24px 48px; max-height: 85vh;
          overflow-y: auto; z-index: 101;
          animation: mcat-slideUp 250ms cubic-bezier(0.32, 0.72, 0, 1);
        }
        .mcat-sheet-handle {
          width: 40px; height: 4px; background: #E5E4E0;
          border-radius: 2px; margin: 0 auto 20px;
        }
        @keyframes mcat-fadeIn { from { opacity: 0 } to { opacity: 1 } }
        @keyframes mcat-slideUp { from { transform: translateY(100%) } to { transform: translateY(0) } }
        .mcat-sort {
          height: 36px; border: 1px solid #E5E4E0; border-radius: 8px;
          padding: 0 32px 0 12px; font-size: 13px;
          font-family: 'DM Sans', sans-serif; color: #2C2C2A;
        ...
        @media (max-width: 1024px) { .mcat-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 768px) {
          .mcat-layout { grid-template-columns: 1fr; padding: 16px 16px 56px; gap: 0; }
          .mcat-sidebar { display: none; }
          .mcat-mobile-btn { display: flex; }
          .mcat-grid { grid-template-columns: repeat(2, 1fr); gap: 12px; }
        }
        @media (max-width: 480px) { .mcat-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; } }
      `}</style>

      {/* Top bar */}
      <div className="mcat-topbar">
        <div className="mcat-topbar-inner">
          <nav style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px" }}>
            ...
            <Link to="/moda" style={{ color: "#7A3048", textDecoration: "none", fontWeight: 500 }}>
              Moda y Accesorios
            </Link>
            <span style={{ color: "#D5D4D0" }}>›</span>
      ...
      {/* Main layout */}
      <div className="mcat-layout">

        {/* Desktop sidebar */}
        <aside className="mcat-sidebar">{sidebarContent}</aside>
        ...
        <div>
          ...
          <h1
            style={{
              fontSize: "24px", fontWeight: 700, color: "#2C2C2A",
              margin: "0 0 20px", letterSpacing: "-0.01em",
            }}
          >
            Catálogo de Moda y Accesorios
          </h1>
          ...
          <div style={{ position: "relative", marginBottom: "20px" }}>
            <Search
              ...
              style={{
                ...
                transform: "translateY(-50%)", color: "#9D9C97", pointerEvents: "none",
              }}
            />
            <input
              type="text"
              ...
              placeholder="Buscar en moda por nombre, categoría..."
              style={{
                width: "100%", height: "44px",
                border: "1.5px solid #E5E4E0", borderRadius: "12px",
                padding: "0 40px 0 40px", fontSize: "14px",
                fontFamily: "'DM Sans', sans-serif", color: "#2C2C2A",
            ...
            {searchQuery && (
              <button
                ...
                style={{
                  ...
                  display: "flex", alignItems: "center", color: "#9D9C97",
                }}
              >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              >
                ...
                {activeCount > 0 && (
                  <span
                    style={{
                      background: "#FFFFFF", color: "#7A3048",
              ...
              <p style={{ fontSize: "14px", color: "#6B6A65", margin: 0, fontWeight: 400 }}>
                <strong style={{ color: "#2C2C2A" }}>{productosFiltrados.length}</strong>{" "}
          ...
          {activeCount > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "12px" }}>
              {categorias.map((cat) => (
                <ActiveChip key={cat} label={cat} onRemove={() => toggleCategoria(cat)} />
              ))}
              {municipiosFiltro.map((id) => (
                <ActiveChip
                  key={id}
                  label={municipiosConProductos.find((m) => m.id === id)?.nombre ?? id}
              ...
              {precioRange && (
                <ActiveChip
                  ...
                  onRemove={() => { setPrecioRange(null); setPrecioDesdeInput(""); setPrecioHastaInput(""); }}
                />
              )}
              {conDescuento && <ActiveChip label="Con descuento" onRemove={() => setConDescuento(false)} />}
              {soloNuevos && <ActiveChip label="Nuevos" onRemove={() => setSoloNuevos(false)} />}
            </div>
          )}
      ...
      {mobileOpen && (
        <>
          <div className="mcat-sheet-backdrop" onClick={() => setMobileOpen(false)} />
          <div className="mcat-sheet">
            <div className="mcat-sheet-handle" />
            <div
              style={{
                display: "flex", alignItems: "center",
            ...
            >
              ...
              <button
                ...
                style={{ background: "none", border: "none", cursor: "pointer", padding: "4px", color: "#6B6A65" }}
              >
            ...
            <button
              ...
              onClick={() => setMobileOpen(false)}
              style={{
                marginTop: "24px", width: "100%", height: "44px",
                background: "#7A3048", border: "none", borderRadius: "12px",
...
function FilterSection({
  ...
  return (
    ...
    >
      <p style={{ fontSize: "11px", fontWeight: 700, color: "#9D9C97", letterSpacing: "0.1em", textTransform: "uppercase", margin: "0 0 14px" }}>
        {title}
      </p>
      {children}
    </div>
    ...
    <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", userSelect: "none" }}>
      <div
        ...
        style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          flexShrink: 0, transition: "background 150ms, border 150ms", cursor: "pointer",
        }}
      >
        {checked && (
      ...
      <span
        onClick={onChange}
        style={{ fontSize: "14px", color: checked ? "#2C2C2A" : "#6B6A65", fontWeight: checked ? 500 : 400, fontFamily: "'DM Sans', sans-serif" }}
      >
        {label}
      </span>
    </label>
  ...
  return (
    <div
      style={{
        ...
        background: "#F9F0F3", border: "1px solid #E8C4D0",
        borderRadius: "20px", padding: "4px 10px 4px 12px",
        fontSize: "12px", fontWeight: 500, color: "#7A3048",
        fontFamily: "'DM Sans', sans-serif",
      }}
    >
      {label}
      <button
        type="button"
        onClick={onRemove}
        style={{ background: "none", border: "none", cursor: "pointer", padding: "0", display: "flex", alignItems: "center", color: "#9D3D5E", marginLeft: "2px" }}
      >
...
function EmptyState({ onClear }: { onClear: () => void }) {
  return (
    <div style={{ textAlign: "center", padding: "64px 24px", color: "#6B6A65", fontFamily: "'DM Sans', sans-serif" }}>
      <p style={{ fontSize: "32px", margin: "0 0 12px" }}>🔍</p>
      <p style={{ fontSize: "16px", fontWeight: 600, color: "#2C2C2A", margin: "0 0 8px" }}>
        Sin resultados
      </p>
      ...
      <button
        ...
        style={{
          ...
          background: "#7A3048", border: "none", borderRadius: "10px",
